...
app.get('/api/stats', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({ success: true, stats: { projects, skills, experiences, messages, unread } });
  } catch (err) {
...
async function seedAdmin() {
  ...
  await Admin.create({ name: 'Admin', email: process.env.ADMIN_EMAIL, password: hash });
  console.log('👤 Admin created:', process.env.ADMIN_EMAIL);

  await Profile.create({
    name: 'Your Name', tagline: 'MERN Stack Developer',
    bio: 'MERN Stack Developer from Surat, Gujarat with 6 months of remote internship experience.',
    location: 'Surat, Gujarat, India', email: 'your@email.com',
